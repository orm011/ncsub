...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        stack = [] # will hold: prestart, instart, and k (length)
        value_range = [min(preorder), max(preorder)] # 2 scans
        min_val = value_range[0]
        val2pos = [-1 for _ in range(value_range[1] - min_val + 1)]
        for i,val in enumerate(inorder): # 1 scan
            val2pos[val - min_val] =  i
        
        nodes = [TreeNode(val=v) for v in preorder] # will use all these nodes. # one scan.
        stack.append((nodes[0], 0, 0, len(preorder))) # dummy with left child.
        while stack:
            (node, prestart, instart, k) = stack.pop()
            #print(f"{prestart=}, {instart=}, {k=}, {parent.val=} {isleft=}")
            split = val2pos[node.val - min_val]
            kprime = split - instart 
            ...
            if kprime > 0: # already default to None, no need to deal with this case.
                ...
                if kprime > 1:
                    stack.append((node.left, prestart + 1, instart, kprime)) 
        
            if k - kprime - 1 > 0:
                node.right = nodes[prestart + kprime + 1]
                if k - kprime - 1 > 1:
                    stack.append((node.right, prestart + kprime + 1, instart + kprime + 1, k - kprime - 1))
        ...
        # within that loop, we only add each preorder position (prestart)
        # at most once to the stack, so the stack at most that size.
        # and the loop can only take that many iterations
        # O(n)
        return nodes[0]