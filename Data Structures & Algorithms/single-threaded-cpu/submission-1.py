...
class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        # time was about 20 mins for it to work.
        ...
        # analysis: initial sort is O(nlogn)
        # then we only process each element once, going from list to heap. thats n*O(logn), and then removed from heap. 
        # total is O(nlogn) 
        if tasks == []:
            return []
