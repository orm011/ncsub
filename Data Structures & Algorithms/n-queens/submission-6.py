...
# row - col maps to bit (row - col) % mod (2*n - 1)
diag2s = 0 # will use bit vectors. 
...
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        ...
        def translate(path):
            output = []
            for c in path:
                output.append(position_strings[c])
            return output
        ...
        def dfs(row) -> None: # appends list  positions possible after here
            # propose a column position for row n - n_left:
            global cols
            global diag1s
            global diag2s

            if row == n: # reached the end, solution complete.
                # path is shared and mutated,
                #  need to copy or will mutate prevous solutions
                solutions.append(translate(path))
            ...
            for col in range(n):
                ...
                if cols & colbit or diag1s & diag1bit or diag2s & diag2bit: 
                    continue # inviable
                else:
                    cols |= colbit
                    diag1s |= diag1bit
        ...
        # final: i tried replacing the string prepend with just appending and 
        # doing one copy. this should reduce the amount of string copying.
