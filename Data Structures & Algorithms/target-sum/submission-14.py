...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        for i in range(1,len(nums)):
            ...
            min_sums[i] = math.gcd(min_sums[i-1], num)
        ...
        def dp(k: int, target: int) -> int: # solution for nums[:k]
            if k == 0: # nums is at least length 1
            ...
            elif target > max_sums[i] or (target < min_sums[i] and target != 0):
                ans = 0
            else:
                ...
                pos_sign = dp(k-1, abs(target - num)) # target can always be the positive
