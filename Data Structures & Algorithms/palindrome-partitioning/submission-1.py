class Solution:
    def partition(self, s: str) -> List[List[str]]:
        ...
        for i in range(1,len(s)+1):
            ...
            # complexity analysis:
            # the outer loop has O(n) iters.
            # the inner loop has O(n) iters as well.
            # the slicing has O(n) runtime.
            # the the equality comparison for reflection is also O(n)
            # finally, the accumulation itself depends on the number of results
            # note we  accumulate about sum(npp) for each entry...
            # at least n^3.

            # note. imagine a single char repeated, every substring is a palindrome.
            # then, the number of elements in the solution is 2**n (can add a split at any point).
            # hence, the best we can do is 2*n, and we are adding a factor here. 
            # of n something.
            # moving on but we could save the palindrome comparisons, eg filling up 
            # if i:j is a palindrome can be done once for cost n**3 separately.
            # it can even be done in n**2
        return actualp[-1]